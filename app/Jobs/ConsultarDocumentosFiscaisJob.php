...
use App\Models\Tenant\ConsultaNfeLog;
...
class ConsultarDocumentosFiscaisJob implements ShouldQueue
{
    ...
    public function __construct(string $organizationId)
    {
        ...
        $this->onQueue('consulta-nfe-' . $organizationId);
    }
    ...
    public function handle()
    {
        ...
        $lockKey = 'consulta_nfe_lock_' . $this->organizationId;

        if (Cache::has($lockKey)) {
            Log::info('Consulta já em andamento para a organização', [
        ...
        try {
            ...
            if (
                $organization->is_enable_nfe_servico &&
                $organization->validade_certificado > now()
            ) {

                Log::info('Organização não está mais elegível para consulta automática', [
                    'organization_id' => $this->organizationId
                ]);
            ...
            $resultado = $service->consultarDocumentosFiscais($organization);

            // Registra o resultado da consulta
            $this->registrarResultadoConsulta($organization, $resultado);

            Log::info('Consulta de documentos fiscais finalizada', [
                'organization_id' => $this->organizationId,
                'success' => $resultado['success']
            ]);
        } catch (Exception $e) {
            Log::error('Erro ao processar consulta de documentos fiscais', [
                'organization_id' => $this->organizationId,
    ...
    private function registrarResultadoConsulta(Organization $organization, array $resultado)
    {
        DB::transaction(function () use ($organization, $resultado) {
            ...
            $organization->ultima_consulta_nfe = now();
            $organization->save();

            // Registra o log de consulta
            ConsultaNfeLog::create([
    ...
    {
        Log::error('Falha definitiva na consulta de documentos fiscais', [
            'organization_id' => $this->organizationId,
            'exception' => $exception->getMessage()
        ]);

        // Libera o lock em caso de falha
        Cache::forget('consulta_nfe_lock_' . $this->organizationId);
    }
}
