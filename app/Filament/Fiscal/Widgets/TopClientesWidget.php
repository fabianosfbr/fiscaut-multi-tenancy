...
class TopClientesWidget extends ChartWidget
{
    ...
    protected function getData(): array
    {
        ...
        $cachedData = AnalyticsCache::retrieve("top_clientes_{$organization->cnpj}_{$activeFilter}", 7200);
        ...
        $labels = collect($dadosClientes)->pluck('nome_cliente')
            ->map(function ($nome) {
                return strlen($nome) > 20 ? substr($nome, 0, 17) . '...' : $nome;
            })
            ->toArray();

        $dadosValores = collect($dadosClientes)->pluck('total_valor')->toArray();
    ...
    protected function getDataFromDatabase($organization, $period): array
    {
        ...
        // Preparar dados para o gráfico
        $labels = $dadosClientes->pluck('nome_cliente')
            ->map(function ($nome) {
                return strlen($nome) > 20 ? substr($nome, 0, 17) . '...' : $nome;
            })
            ->toArray();

        $dadosValores = $dadosClientes->pluck('total_valor')->toArray();
